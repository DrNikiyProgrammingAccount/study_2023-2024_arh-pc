...
![Сборка исполняемого файла из lab6-1.asm](image/6.png)
...
![Вставка кода из листинга в созданный ранее файл](image/17.png)
...
![Редактирование файла lab6-3.asm](image/19.png)
...
![Вставка кода из листинга в файл variant.asm](image/22.png)

Соберём и запустим программу, указав номер студенческого билета. В моём случае это 1132231848 (Рис. 2.24):

![Сборка и запуск программы, а также результат выполнения](image/23.png)

<p><br>Программа вывела число 9. Действительно, ведь остаток от деления числа 1132231848 на 20 равен 8 (ведь число 1132231800 делится на 20 без остатка, тогда остаётся найти остаток от деления 48 на 20, что является 8). 8 + 1 = 9, соответственно.
...
Теперь в качестве самостоятельной работы напишем код программы для вычисления выражения в варианте 9: 10 + (31x − 5). В предварительно созданном файле task9.asm впишем следующий код (Рис. 3.1):

![Код требуемой программы](image/24.png)

<p><br>Он немного отличается от предыдущих программ. За вычисление выражения отвечают следующие команды:
<p><br>mul ebx
<p><br>sub eax, 5
<p><br>add eax, 10
<p><br>Где первая отвечает за умножение значения регистра eax (где хранится х) на значение регистра ebx (куда предварительно положили значение 31).
<p><br>Вторая строка отвечает за вычитание из регистра eax пятёрки. 
<p><br>Третья строка отвечает за прибавление к регистру eax десятки.
<p><br>Попробуем собрать нашу программу (Рис. 3.2):

![Сборка исполняемого файла](image/25.png)
